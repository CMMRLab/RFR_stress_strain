...
if __name__ == "__main__":
    ...
    xlimits = (np.min(strain)-delta, np.max(strain)+1.5*delta+0.15)
    ...
    # Start plotting data
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(5, 3*3))
    ...
    if str(wn).startswith('op'):
        ...
        ax1.stem(np.real(freq), psd_stress, linefmt=color, basefmt=color, markerfmt='.', label='$|X(f)|^2/N$')
        ax1.axhline(mean_stress_psd, color='#ff9d3aff', ls='--', lw=2, label='Average power={:.4f}'.format(mean_stress_psd))
        ...
        ax1.set_xlim((-1, 250)) # Comment/uncomment for xlimits
        ax1.set_ylim((-1*mean_stress_psd, 30*mean_stress_psd)) # Comment/uncomment for xlimits
        ax1.text(*label_rel_pos, '(a)', transform=ax1.transAxes, fontsize=fs, fontweight='bold', va='top', ha='left')
        


    color_index = 0
    ax2.plot(strain, stress, '.', ms=4, color='#bbbbbbff', label='LAMMPS data')
    ...
    for i in range(lo, hi+1):
        ...
        else: label += ' (Amplitude={:>6.2f})'.format(amplitude)
        color, color_index = walk_colors(color_index, colors)
        
        ax2.plot(strain, wave, '-', lw=2, color=color, label=label)
        ax1.plot(freq[i], psd_stress[i], 'o', ms=8, color=color)
    ax2.plot(strain, comp_wave, '-', lw=4, color='#2c7fb8ff', label='Fourier filter (indexes {}-{})'.format(lo, hi))
    ...
    for i in range(lo, hi+1):
        ...
        comp_wave += wave
        label = 'PSD index: {:<2}'.format(i)
        label += ' (Amplitude={:>6.2f})'.format(amplitude)
        color, color_index = walk_colors(color_index, colors)
        ax3.plot(strain, wave, '-', lw=2, color=color, label=label)
        ax1.plot(freq[i], psd_stress[i], 'o', ms=8, color=color)
    ax3.plot(strain, comp_wave, '-', lw=4, color='#2c7fb8ff', label='Fourier filter (indexes {}-{})'.format(lo, hi))
    ...
    fig.tight_layout()
    plt.show()
    basename = logfile[:logfile.rfind('.')] + '_Fourier_Filter'

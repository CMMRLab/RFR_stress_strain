...
if __name__ == "__main__":
    ...
    ax1.plot(strain, stress, '.', ms=4, color='tab:cyan', label='LAMMPS data')
    ax1.plot(strain, filtered_stress, '-', lw=2, color='tab:blue', label='Filtered data')
    if x_yield_d2 is not None and y_yield_d2 is not None:
        ax1.plot(x_yield, y_yield, 'o', ms=8, color='tab:green', label='Yield point ({:.4f}, {:.4f})'.format(x_yield, y_yield))
    ax1.plot(youngs_modulus_x, youngs_modulus_y, '-', lw=4, color='tab:orange', label="Young's modulus={:.4f}".format(youngs_modulus_coeffs[1]))
    ax1.legend(loc='lower right', bbox_to_anchor=(1, 0), fancybox=True, ncol=1, fontsize=fs)
    ax1.set_xlabel('True Strain', fontsize=fs)
    ax1.set_ylabel('True Stress (MPa)', fontsize=fs)
    ax1.tick_params(axis='both', which='major', labelsize=fs)
    
    ax2.plot(strain, trans1, '.', ms=4, color='tab:cyan', label='LAMMPS data ({})'.format(trans1_column))
    ax2.plot(strain, filtered_trans1, '-', lw=2, color='tab:blue', label='Filtered data ({})'.format(trans1_column))
    ax2.plot(nu_1_x, nu_1_y, '-', lw=4, color='tab:orange', label="Poisson's ratio={:.4f}".format(-nu_1_coeffs[1]))
    ax2.legend(loc='lower right', bbox_to_anchor=(1, 0), fancybox=True, ncol=1, fontsize=fs)
    ax2.set_xlabel('True strain', fontsize=fs)
    ax2.set_ylabel('Transverse strain', fontsize=fs)
    ax2.tick_params(axis='both', which='major', labelsize=fs)
    
    ax3.plot(strain, trans2, '.', ms=4, color='tab:cyan', label='LAMMPS data ({})'.format(trans2_column))
    ax3.plot(strain, filtered_trans2, '-', lw=2, color='tab:blue', label='Filtered data ({})'.format(trans2_column))
    ax3.plot(nu_2_x, nu_2_y, '-', lw=4, color='tab:orange', label="Poisson's ratio={:.4f}".format(-nu_2_coeffs[1]))
    ax3.legend(loc='lower right', bbox_to_anchor=(1, 0), fancybox=True, ncol=1, fontsize=fs)
    ax3.set_xlabel('True strain', fontsize=fs)
    ax3.set_ylabel('Transverse strain', fontsize=fs)
    ax3.tick_params(axis='both', which='major', labelsize=fs)
    
    if str(wn).startswith('op'):
        ax4.stem(wns_stress, psd_stress, linefmt='tab:blue', markerfmt='.', label='$|X(f)|^2/N$ for stress-strain')
        ax4.plot(wn_stress, power_stress, 'o', ms=8, color='tab:green', label='Critical frequency\n({:.4f}, {:.4f})'.format(wn_stress, power_stress))
        ax4.axhline(mean_stress_psd, color='tab:orange', ls='--', lw=2, label='Mean power={:.4f}'.format(mean_stress_psd))
        ax4.set_xlabel('Normalized Frequencies (unitless)', fontsize=fs)
        ax4.set_ylabel('Power Spectral Density', fontsize=fs)
        ax4.legend(loc='upper center', bbox_to_anchor=(0.5, 1.0), fancybox=True, ncol=1, fontsize=fs)
        ax4.tick_params(axis='both', which='major', labelsize=fs)
        ax4.set_xlim((-0.001, 0.02)) # Comment/uncomment for xlimits
        ax4.set_ylim((-1*mean_stress_psd, 25*mean_stress_psd)) # Comment/uncomment for xlimits
        
    ax5.plot(fr1_fringe, fr1_slopes, '-', lw=2, color='tab:cyan', label='1st forward fringe response')
    ax5.plot(fr1_max_fringe, fr1_max_slope, 'o', ms=8, color='tab:blue', label='Maximum 1st forward')
    ax5.plot(br1_fringe, br1_slopes, '-', lw=2, color='lime', label='1st backward fringe response')
    ax5.plot(br1_max_fringe, br1_max_slope, 'o', ms=8, color='tab:green', label='Maximum 1st backward')
    ax5.plot(fr2_fringe, fr2_slopes, '-', lw=2, color='violet', label='2nd forward fringe response')
    ax5.plot(fr2_max_fringe, fr2_max_slope, 'o', ms=8, color='tab:purple', label='Maximum 2nd forward')
    ax5.legend(loc='lower right', bbox_to_anchor=(1, 0), fancybox=True, ncol=1, fontsize=fs)
    ax5.set_xlabel('True Strain', fontsize=fs)
    ax5.set_ylabel('Fringe response (MPa)', fontsize=fs)
    ...
    if x_yield_d2 is not None and y_yield_d2 is not None:
        ax6.plot(x_yield_d2, y_yield_d2, 'o', ms=8, color='tab:green', label='Yield point ({:.4f}, {:.4f})'.format(x_yield_d2, y_yield_d2))
    ...
    ax6.legend(loc='lower right', bbox_to_anchor=(1, 0), fancybox=True, ncol=1, fontsize=fs)
    ax6.set_xlabel('True Strain', fontsize=fs)
    ax6.set_ylabel('$d^2(Fringe response)/d(strain)^2$ (MPa)', fontsize=fs)
